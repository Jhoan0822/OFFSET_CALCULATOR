...
export const exportToPDF = async (
    ...
    try {
        ...
        if (logoElement) {
            ...
            const imgData = canvas.toDataURL('image/png'); // Obtenemos la imagen como base64
            
            // CORRECCIÓN: Usamos html2canvas para el logo para consistencia
            const imgProps = doc.getImageProperties(imgData);
            const pdfWidth = doc.internal.pageSize.getWidth() - 2 * margin;
            const pdfHeight = (imgProps.height * pdfWidth) / imgProps.width;

            // CORRECCIÓN CLAVE: Le decimos a jsPDF que el formato es 'PNG'
            doc.addImage(imgData, 'PNG', margin, y, pdfWidth, pdfHeight);
            y += pdfHeight + 20;
        }

        // ... (El resto del texto del PDF no cambia)
        ...
        if (visualizerElement) {
            ...
            const vizImgData = vizCanvas.toDataURL('image/png'); // Obtenemos la imagen como base64
            ...
            // CORRECCIÓN CLAVE: Especificamos el formato 'PNG' aquí también
