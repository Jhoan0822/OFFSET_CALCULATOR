...
export const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ quote, formState }) => {
    ...
    const handleShareToWhatsApp = () => {
        const message = [
            ...
            textData.dimensions, textData.quantity, textData.paper,
            textData.sheet, textData.printing, textData.finishes,
            '',
            `*${textData.costBreakdownTitle}*`,
            textData.itemsPerSheet, textData.sheetsWithWaste, textData.paperCost,
    ...
    return (
        // El id="print-area" se aplica a todo el div para imprimirlo completo
        <div id="print-area"> 
            <Card className="space-y-6" ref={resultsCardRef}>
                <div>
                    ...
                    {hasResults ? (
                        ...
                        <div className="text-center py-8">{/* Mensaje de error si no cabe */}</div>
                    )}
                </div>
            </Card>
        </div>
